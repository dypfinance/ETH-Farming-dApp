...
export default class StakingStats extends React.Component {
    ...
    constructor(props) {
        ...
        window.LP_ID_LIST_V2.forEach(lp_id => this.state.pools_info[lp_id] = {})
    }
                                    ...
                                    window.LP_ID_LIST_V2.map((lp_id, i) => {
                                        ...
                                        return (
                                            <tr key={lp_id} style={{visibility: displayNone, display: displayNone2}}>
                                                ...
                                                <td className={Number(depositedLp) > 0 ? 'text-bold' : 'text-muted'}> {f( depositedLp/1e18 * window.rebase_factors[0], 6)} (${f( depositedLp/1e18*usd_per_lp, 2 )}) </td>
                                                ...
                                                {/*Deposited DYP*/}
                                                <td className={Number(depositedLp) > 0 ? 'text-bold' : 'text-muted'}> {f( depositedDyp/1e18 * window.rebase_factors[0], 2)} (${f( depositedDyp/1e18*usd_per_token, 2 )}) </td>
