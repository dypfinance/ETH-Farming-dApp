...
class TOKEN {
    ...
    async transfer(to, amount) {
        ...
        return (await contract.methods.transfer(to, amount).send({gas, from: await getCoinbase(), maxFeePerGas: latestGasPrice, maxPriorityFeePerGas: maxPriorityFeePerGas }))
    }
    async totalSupply() {
        let contract = await getContract(this.key)
    ...
    async approve(spender, amount) {
        ...
        return (await contract.methods.approve(spender, amount).send({gas, from: await getCoinbase(), maxFeePerGas: latestGasPrice, maxPriorityFeePerGas: maxPriorityFeePerGas }))
    }
...
async function approveToken(token, spender, amount) {
    ...
    return await tokenContract.methods.approve(spender, amount).send({maxFeePerGas: latestGasPrice, maxPriorityFeePerGas: maxPriorityFeePerGas})
}
